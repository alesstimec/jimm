...
func (d *Dialer) dial(ctx context.Context, ctl *dbmodel.Controller, connModelTag names.ModelTag, user *openfga.User, loginRequest *jujuparams.LoginRequest) (_ *Connection, err error) {
	...
	defer func() {
		span.Finish(err,
			...
			jujuTrace.StringAttr("model.uuid", connModelTag.Id()),
		)
	}()
...
func (c *Connection) APICall(ctx context.Context, objType string, version int, id, request string, params, response any) error {
...
func (c *Connection) ConnectStream(ctx context.Context, path string, attrs url.Values) (base.Stream, error) {
	...
	requestHeader, err := c.authorizationHeader(ctx, modelTag, nil)
	if err != nil {
		return nil, err
	}
...
func (c *Connection) ConnectControllerStream(ctx context.Context, path string, attrs url.Values, extraHeaders http.Header) (base.Stream, error) {
	header, err := c.authorizationHeader(ctx, names.ModelTag{}, extraHeaders)
	if err != nil {
		return nil, err
	}
