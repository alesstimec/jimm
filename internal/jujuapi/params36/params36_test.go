...
func TestCurrentModelCreateArgs(t *testing.T) {
	c := qt.New(t)
	...
	got, err := CurrentModelCreateArgs(in)
	c.Assert(err, qt.IsNil)
	...
	got, err = CurrentModelCreateArgs(jujuparams.ModelCreateArgsLegacy{Name: "m"})
	c.Assert(err, qt.IsNil)
	c.Check(got.Qualifier, qt.Equals, "")

	// An invalid owner tag is an error.
	_, err = CurrentModelCreateArgs(jujuparams.ModelCreateArgsLegacy{Name: "m", OwnerTag: "not-a-tag"})
...
func TestCurrentOfferFilters(t *testing.T) {
	c := qt.New(t)
